...
<body>
<div class="adm-shell">
  @include('admin.partials.sidebar', ['currentPage' => 'update_sekolah', 'currentModule' => 'angkutansekolah'])
  ...
  <main class="adm-main">
    <div class="adm-content">
      ...
      <div class="info-panel">
        ...
        <div>
          ID sekolah <strong>digenerate otomatis</strong>. Nama sekolah dikonversi ke <strong>HURUF KAPITAL SEMUA</strong> secara otomatis saat menyimpan.
        </div>
      </div>
      ...
      <div class="data-card">
        ...
        <div class="new-row-section">
          <div class="new-row-label">+ Tambah Sekolah Baru</div>
          <div class="new-row-grid">
            <div class="field-group">
              <span class="field-label">Nama Sekolah</span>
              <input type="text" id="nSekolah" class="edit-input" placeholder="Contoh: SMAN 1 TULUNGAGUNG"
...
// ── ID generator (5-digit numeric like existing data) ──────────────
function genId() {
...
async function loadData() {
  ...
  try {
    allData = await sbGetShared('sekolah', 'select=id,sekolah&order=sekolah.asc');
...
function getFiltered() {
  ...
  if (!q) return allData;
  return allData.filter(r => (r.sekolah||'').toLowerCase().includes(q) || String(r.id).includes(q));
}
...
function renderTable() {
  ...
  tbody.innerHTML = slice.map((row, i) => `
    <tr id="row_${esc(row.id)}" data-id="${esc(row.id)}">
      ...
      <!-- View mode -->
      <td>
        <span class="vv-text">${esc(row.sekolah)}</span>
        <input type="text" class="edit-input vv-edit" style="display:none"
          value="${esc(row.sekolah)}" maxlength="120"
          autocomplete="off" oninput="this.value=this.value.toUpperCase()"/>
      </td>
      ...
      <td class="td-actions">
        <div class="vv-view-btns" style="display:flex;gap:5px">
          ...
          <button class="btn-del" onclick="deleteRow('${esc(row.id)}', '${esc(row.sekolah)}')">
...
function cancelEdit(id) {
  ...
  if (orig) row.querySelector('.vv-edit').value = orig.sekolah;
...
async function saveRow(id) {
  ...
  if (!val) { showToast('Nama sekolah tidak boleh kosong.', 'error'); input.focus(); return; }
  ...
  try {
    await sbPatch('sekolah', `id=eq.${encodeURIComponent(id)}`, { sekolah: val });
    const idx = allData.findIndex(r => String(r.id) === String(id));
    if (idx !== -1) allData[idx].sekolah = val;
...
async function deleteRow(id, nama) {
  if (!(await confirmDangerModal(`Hapus sekolah "${nama}"?\n\nPastikan tidak ada siswa yang masih terdaftar di sekolah ini.`))) return;
  try {
    await sbDelete('sekolah', `id=eq.${encodeURIComponent(id)}`);
...
async function addRow() {
  const input = document.getElementById('nSekolah');
  const val   = input.value.trim().toUpperCase();
  if (!val) { showToast('Nama sekolah tidak boleh kosong.', 'error'); input.focus(); return; }

  // Check duplicate
  if (allData.some(r => r.sekolah === val)) {
    showToast(`Sekolah "${val}" sudah ada.`, 'error'); input.focus(); return;
  }
  ...
  try {
    const newId = genId();
    await sbPost('sekolah', { id: newId, sekolah: val });
    allData.push({ id: newId, sekolah: val });
    allData.sort((a,b) => a.sekolah.localeCompare(b.sekolah));
    document.getElementById('totalCount').textContent = allData.length;
    input.value = '';
    ...
    renderTable();
    showToast(`✓ Sekolah "${val}" berhasil ditambahkan.`);
  } catch(e) {
...
if (sbRealtime) {
  sbRealtime
    .channel('admin-data-sekolah')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'sekolah' }, scheduleReload)
    .subscribe();
}
