...
<style>
  ...
  .data-table td { padding:10px 14px; font-size:13px; color:var(--text-1); vertical-align:middle; }

  .name-badge { font-weight:600; color:var(--text-1); }
  .coord-empty { color:var(--text-4); font-style:italic; font-size:12.5px; }
  .coord-val { font-family:'DM Mono',monospace; font-size:12.5px; color:var(--text-2); }

  .edit-input { width:100%; padding:6px 10px; border:1.5px solid var(--border); border-radius:7px; font-family:'DM Mono',monospace; font-size:12.5px; color:var(--text-1); background:var(--surface); outline:none; transition:border-color .15s,box-shadow .15s; box-sizing:border-box; }
  .edit-input:focus { border-color:var(--accent); box-shadow:0 0 0 3px var(--accent-glow); }
  .coord-split { display:flex; align-items:center; gap:6px; }
  .coord-split .cfld { flex:1; min-width:90px; }

  .td-actions { white-space:nowrap; }
  .btn-edit, .btn-save-row, .btn-cancel-row {
  ...
  .btn-map { display:inline-flex; align-items:center; gap:4px; font-size:11.5px; color:var(--accent); text-decoration:none; font-weight:600; margin-left:8px; }
...
<body>
<div class="adm-shell">
  @include('admin.partials.sidebar', ['currentPage' => 'asdp_koordinat', 'currentModule' => 'asdp'])
  ...
  <main class="adm-main">
    <div class="adm-content">
      ...
      <div class="info-panel">
        ...
        <div>
          Nama lokasi di sini <strong>tidak bisa diedit</strong> (kelola nama &amp; foto lewat menu <strong>Daftar Lokasi ASDP</strong>). Gunakan koordinat desimal, mis. <span class="coord-val">-8.0654</span> untuk lat dan <span class="coord-val">111.9022</span> untuk lng. Data dibaca lewat <strong>cache bersama</strong> dengan halaman peta ASDP publik.
        </div>
      </div>
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th>Nama Lokasi</th>
              <th style="width:280px">Koordinat (Lat, Lng)</th>
...
// ── Validasi koordinat desimal (boleh kosong, boleh negatif) ───────
function validCoord(val, min, max) {
  if (val === '') return true; // boleh dikosongkan (belum diatur)
...
async function loadData() {
  ...
  try {
    // Query SAMA PERSIS dengan daftarlist.php & halaman publik peta ASDP
    // supaya berbagi 1 file cache yang sama (lihat catatan lengkap di
    // loadData() pada admin/pages/asdp/daftarlist.php). Kolom lat/lng
    // tetap ikut terbawa karena select default-nya '*'.
...
function getFiltered() {
  ...
  if (!q) return allData;
  return allData.filter(r => (r.name||'').toLowerCase().includes(q));
}
...
function renderTable() {
  ...
  tbody.innerHTML = slice.map((row, i) => {
    const hasCoord = row.lat && row.lng;
    const mapLink  = hasCoord ? `https://www.google.com/maps?q=${encodeURIComponent(row.lat)},${encodeURIComponent(row.lng)}` : '';
    return `
    <tr id="row_${esc(row.id)}" data-id="${esc(row.id)}">
      <td style="color:var(--text-4);font-size:12px">${start + i + 1}</td>
      <td><span class="name-badge">${esc(row.name)}</span></td>

      <!-- Koordinat -->
      <td>
        <span class="vv-view">
          ${hasCoord
            ? `<span class="coord-val">${esc(row.lat)}, ${esc(row.lng)}</span><a class="btn-map" href="${mapLink}" target="_blank" rel="noopener">Lihat peta ↗</a>`
            : `<span class="coord-empty">Belum diatur</span>`
          }
        </span>
        <div class="vv-edit coord-split" style="display:none">
          <input type="text" class="edit-input cfld ei-lat" value="${esc(row.lat || '')}" placeholder="Lat, mis -8.0654"
            autocomplete="off" inputmode="decimal"/>
          <input type="text" class="edit-input cfld ei-lng" value="${esc(row.lng || '')}" placeholder="Lng, mis 111.9022"
            autocomplete="off" inputmode="decimal"/>
        </div>
      </td>
...
function startEdit(id) {
  ...
  row.querySelector('.vv-view').style.display = 'none';
  row.querySelector('.vv-edit').style.display = 'flex';
  row.querySelector('.vv-view-btns').style.display = 'none';
  row.querySelector('.vv-edit-btns').style.display = 'inline-flex';
  row.querySelector('.ei-lat').focus();
}
...
function cancelEdit(id) {
  ...
  if (orig) {
    row.querySelector('.ei-lat').value = orig.lat || '';
    row.querySelector('.ei-lng').value = orig.lng || '';
  }
  row.querySelector('.vv-view').style.display = '';
  row.querySelector('.vv-edit').style.display = 'none';
  row.querySelector('.vv-view-btns').style.display = '';
  row.querySelector('.vv-edit-btns').style.display = 'none';
}

async function saveEdit(id) {
  const row = document.getElementById(`row_${id}`);
  const lat = row.querySelector('.ei-lat').value.trim();
  const lng = row.querySelector('.ei-lng').value.trim();

  if (!validCoord(lat, -90, 90))  { showToast('Latitude tidak valid (harus -90 s/d 90)', 'error'); return; }
  if (!validCoord(lng, -180, 180)) { showToast('Longitude tidak valid (harus -180 s/d 180)', 'error'); return; }
  ...
  try {
    await sbPatch('list_tambangan', `id=eq.${encodeURIComponent(id)}`, { lat, lng, updated: nowIso() });
    const rec = allData.find(r => String(r.id) === String(id));
    if (rec) { rec.lat = lat; rec.lng = lng; }
