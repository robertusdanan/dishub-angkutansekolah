...
<head>
  ...
  <link rel="canonical" href="{{ url('/absenrfid/listlink') }}"/>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .user-badge svg { color: rgba(212,175,55,.6); flex-shrink: 0; }
    .user-badge .uname { color: #fff; font-weight: 600; }
    ...
    @media (max-width: 480px) {
      .user-bar { top: 10px; right: 10px; gap: 6px; }
      .user-badge { padding: 5px 10px 5px 8px; font-size: 11px; }
      .btn-logout { padding: 5px 10px 5px 8px; font-size: 11px; }
    }
  </style>
</head>
<body>
  ...
  <div class="user-bar">
    <div class="user-badge">
      ...
      <span class="uname">{{ $listlinkUser }}</span>
    </div>
    <a href="/absenrfid/logout" class="btn-logout" title="Keluar">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2">
    ...
    async function loadDrivers() {
      ...
      try {
        ...
        const busDrivers = driverBusRows
          ...
          .map(d => ({ id: d.id, nama: d.driver, link: `/absenrfid?id=${d.id}`, jenis: 'bus' }));

        const mpuDrivers = driverMpuRows
          .filter(d => d.id && d.driver)
          .map(d => ({ id: d.id, nama: d.driver, link: `/absenrfid?id=${d.id}`, jenis: 'mpu' }));
